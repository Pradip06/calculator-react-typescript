        ...
        {rows.map((row, i) => {
          return (
            <Fragment key={row.toString()}>
              <div role="row" >
                {i === 3 && <button className="btn"  onClick={clearValue}>{clear}</button>}
                {row.map((n) => (
                ...
                {i === 3 && <button className="btn" onClick={calculate}>{equalSign}</button>}
              </div>
            </Fragment>
          );
        })}
        {calcOperators.map((c) => (
          <button className="btn"  key={c} onClick={() => setValue(value.concat(c))}>
            {c.toString()}
          </button>
        ))}
      </div>
